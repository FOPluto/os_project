...
ArmorDetector::ArmorDetector()://  1
    warpPerspective_mat(cv::Mat::zeros(3,3,CV_32FC1)),
...
*/
void ArmorDetector::InitArmor(std::string xml_path, std::string bin_path)
{
    //cout<<"装甲板检测初始化成功！"<<endl;
    //！std::cout << "armor_detector init begin" << std::endl;

    std::string file_path="/root/os_project/xml_path/armor_limited.xml";
    cv::FileStorage fr;
    ...
    // 初始化深度学习算法类
    this->yolov5_detector_ = new Yolov5(xml_path, bin_path, 416, 416); // 创建yolov5detector对象
    cout << yolov5_detector_->m_bin_path << endl;
 ...
 */
void ArmorDetector::Yolov2Res(cv::Mat &frame){
    match_armors_ = yolov5_detector_->detect_yolov5(frame); // 模型推理到结果
}
